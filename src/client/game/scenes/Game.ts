...
export class Game extends Scene {
  ...
  create() {
    ...
    this.shareButton.on('pointerdown', async () => {
      ...
      this.game.renderer.snapshot(async (snapshot: HTMLImageElement | Phaser.Display.Color) => {
        this.hidingSpot.setVisible(true);

        // Type guard to ensure we have an HTMLImageElement
        if (!(snapshot instanceof HTMLImageElement)) {
          console.error('Snapshot is not an HTMLImageElement');
